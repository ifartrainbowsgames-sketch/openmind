...
export function parseStructuredPlan(text: string, allowedTools: string[]): PlanStep[] {
...
export function parsePlannerOutput(text: string, allowedTools: string[]): PlanStep[] {
  const structured = parseStructuredPlan(text, allowedTools)
  return structured.length ? structured : parsePlan(text, allowedTools)
}
